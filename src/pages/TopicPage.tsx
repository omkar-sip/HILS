...
export default function TopicPage() {
    ...
    const topicContext = useMemo(() => {
        if (topicId === 'custom') {
            return {
                topic: { id: 'custom', name: 'Custom Question / PYQ', order: 0, description: '' },
                module: { id: 'm-custom', name: 'Custom Subject Area', moduleNumber: 0, order: 0, topics: [] },
                subject: { id: 's-custom', name: 'Ad-hoc Query', code: 'CUSTOM', credits: 0, isElective: false, isLab: false, modules: [] },
                semester: { id: 'sem-custom', number: 0, schemeYear: '2022', subjects: [] },
                prevTopic: null,
                nextTopic: null,
                totalInModule: 1,
                positionInModule: 1,
    ...
    useEffect(() => {
        if (topicId && topicId !== 'custom') {
            setTopic(topicId)
        }
        if (topicId === 'custom' || initialQuery) {
            setMode('exam_answer')
        }
    }, [topicId, initialQuery, setTopic, setMode])
    ...
    const handleGenerate = useCallback(() => {
        ...
        let finalContext = topic.description || ''
        if (customQuery.trim()) {
            finalContext = `[USER SPECIFIC QUERY/PYQ OVERRIDE]\n${customQuery}\n\n[ORIGINAL TOPIC CONTEXT]\n${finalContext}`
        }

        generateExplanation({
            topicId: topic.id,
            topicName: topic.id === 'custom' ? 'Custom Query' : topic.name,
            subjectName: subject.name,
            moduleName: mod.name,
            personaId: activePersona.id,
            personaModifier: activePersona.promptModifier,
            mode: activeMode,
            syllabusContext: finalContext,
        })
    }, [topicContext, customQuery, activePersona, activeMode, generateExplanation])

    // Auto-generate on mount if there's an initial query
    const hasAutoGenerated = useRef(false)
    useEffect(() => {
        // Trigger if there's a custom query in the URL, we're on the custom page, and we haven't generated yet
        if (initialQuery && topicId === 'custom' && activeMode === 'exam_answer' && !hasAutoGenerated.current) {
            hasAutoGenerated.current = true;
            // Slight delay to ensure mode is set and UI is ready
            setTimeout(() => {
                handleGenerate();
            }, 100);
        }
    }, [initialQuery, topicId, activeMode, handleGenerate])
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 md:p-6">
            ...
            {/* Custom PYQ Override */}
            <div className="mb-6">
                <div className="glass-card p-4 border border-hils-accent/20 border-l-4 border-l-hils-accent-light bg-hils-accent/5">
                    <label className="flex items-center gap-2 text-xs font-semibold text-hils-accent-light mb-2 uppercase tracking-wide">
                        <Zap className="w-3 h-3 text-hils-accent-light" />
                        I have a PYQ / Custom Question
                    </label>
                    <textarea
                        value={customQuery}
                        onChange={(e) => setCustomQuery(e.target.value)}
                        placeholder="Paste your exact exam question here to override standard syllabus context..."
                        className="w-full bg-black/40 border border-hils-border rounded-lg p-3 text-sm text-hils-text placeholder:text-hils-text-dim focus:outline-none focus:border-hils-accent/50 transition-colors min-h-[80px] resize-y"
                    />
                </div>
            </div>
